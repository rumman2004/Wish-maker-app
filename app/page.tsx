    ...
    <main className="min-h-screen flex flex-col items-center justify-center p-3 sm:p-4 md:p-6 transition-colors duration-500 font-sans selection:bg-indigo-500/30">
      ...
      <div className="w-full max-w-lg flex justify-between items-center mb-4 sm:mb-6 md:mb-10">
         <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
            <div className="neu-flat p-2 sm:p-3 md:p-4 rounded-xl md:rounded-2xl text-indigo-500 dark:text-indigo-400">
        ...
        {!wishId ? (
          <div className="space-y-4 sm:space-y-6 md:space-y-10">
            ...
            <div className="text-center">
              <p className="text-[9px] sm:text-[10px] md:text-xs font-extrabold uppercase tracking-widest opacity-40 mb-3 sm:mb-4 md:mb-6 dark:text-slate-500">Select Vibe</p>
                ...
                {Object.entries(THEMES).map(([key, theme]) => (
                  <div 
                    ...
                    className={`
                      ...
                      ${formData.theme === key 
                        ? 'border-indigo-500 dark:border-indigo-400 neu-pressed' 
                  ...
                  >
                    ...
                    <p className={`text-[7px] sm:text-[8px] md:text-[10px] font-bold uppercase tracking-wider truncate px-0.5 sm:px-1 ${formData.theme === key ? 'text-indigo-600 dark:text-indigo-300' : 'opacity-50 dark:text-slate-400'}`}>
                      {theme.label}
                    </p>
                    ...
                    <button 
                      ...
                      className="absolute -top-1.5 -right-1.5 sm:-top-2 sm:-right-2 md:-top-3 md:-right-3 bg-indigo-500 text-white dark:bg-indigo-600 rounded-full p-1 sm:p-1.5 md:p-2 shadow-lg hover:scale-110 transition z-10"
                    >
            ...
            <div className="space-y-3 sm:space-y-4 md:space-y-6">
              <div className="space-y-1 sm:space-y-1.5 md:space-y-2 relative">
                <label className="text-[9px] sm:text-[10px] md:text-xs font-bold uppercase ml-3 sm:ml-4 opacity-40 dark:text-slate-500">Recipient</label>
                <div className="relative">
                  ...
                  <input 
                    className="w-full p-2.5 pl-9 sm:p-3.5 sm:pl-10 md:p-5 md:pl-14 neu-pressed text-sm sm:text-base md:text-lg font-medium text-slate-700  placeholder:opacity-30 dark:bg-[#1A1B1E]"
              ...
              <div className="space-y-1 sm:space-y-1.5 md:space-y-2 relative">
                ...
                <div className="relative">
                  ...
                  <textarea 
                    className="w-full p-2.5 pl-9 sm:p-3.5 sm:pl-10 md:p-5 md:pl-14 neu-pressed text-sm sm:text-base md:text-lg font-medium min-h-[100px] sm:min-h-[120px] md:min-h-[160px] resize-none text-slate-700  placeholder:opacity-30 dark:bg-[#1A1B1E]"
              ...
              <div className="space-y-1 sm:space-y-1.5 md:space-y-2 relative">
                 <label className="text-[9px] sm:text-[10px] md:text-xs font-bold uppercase ml-3 sm:ml-4 opacity-40 dark:text-slate-500">Privacy (Optional)</label>
                 <div className="relative">
            ...
            <button 
              ...
              disabled={loading}
              className="w-full py-3 sm:py-4 md:py-6 rounded-xl sm:rounded-2xl md:rounded-3xl neu-flat neu-btn text-indigo-600 dark:text-indigo-400 text-sm sm:text-base md:text-lg tracking-widest hover:text-indigo-700 dark:hover:text-indigo-300 disabled:opacity-50 font-black uppercase"
            >
          ...
          <div className="text-center py-4 sm:py-6 md:py-10">
            <motion.div 
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
              className="w-16 h-16 sm:w-20 sm:h-20 md:w-28 md:h-28 rounded-full neu-flat flex items-center justify-center mx-auto mb-4 sm:mb-6 md:mb-8 text-green-500 dark:text-green-400"
            >
              <Check size={24} className="sm:w-8 sm:h-8 md:w-12 md:h-12" strokeWidth={4} />
            </motion.div>
            
            <h2 className="text-xl sm:text-2xl md:text-3xl font-black mb-1.5 sm:mb-2 text-slate-800 dark:text-white">It's Ready!</h2>
            <p className="opacity-60 mb-6 sm:mb-8 md:mb-10 text-xs sm:text-sm md:text-base text-slate-600 dark:text-slate-400 font-medium">Your wish has been wrapped and sealed.</p>
            
            <motion.div 
               initial={{ opacity: 0, y: 10 }}
               animate={{ opacity: 1, y: 0 }}
               className="neu-pressed p-3 sm:p-4 md:p-5 rounded-xl sm:rounded-2xl flex items-center gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6 md:mb-8 overflow-hidden bg-opacity-50 dark:bg-[#1A1B1E]"
            >
               <LinkIcon size={14} className="sm:w-4 sm:h-4 md:w-[18px] md:h-[18px] text-slate-400 shrink-0" />
               <span className="text-[10px] sm:text-xs md:text-sm opacity-60 truncate flex-1 text-left text-slate-700 dark:text-slate-300 font-mono">
            ...
            <motion.button 
              ...
              className="w-full py-3 sm:py-4 md:py-6 rounded-xl sm:rounded-2xl neu-flat neu-btn mb-4 sm:mb-6 font-bold text-slate-700 dark:text-slate-200 text-sm sm:text-base md:text-lg"
            >
              {copied ? (
